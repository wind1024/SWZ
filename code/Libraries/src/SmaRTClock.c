...
void RTC_Init (void)
{
   ...
   //----------------------------------------  
      ...
      RTC_Write (RTC0XCF, 0x80);       // Disable Automatic Load Capacitiance                                       
   ...
   CLKSEL    =  0x74;                  // Switch to 156 kHz low power
      ...
      // Wait for smaRTClock valid 检测CLKVLD：smaRTClock振荡器晶体有效标志位
      while ((RTC_Read (RTC0XCN) & 0x10)== 0x00);

      // Wait for Load Capacitance Ready检测 LOADRDY：负载电容准备好标志 
...
U8 RTC_Read (U8 reg)
{
   while(RTC0ADR & 0x80);			   //查询 BUSY（RTC0ADR.7）直到其返回0值
...
void RTC_Write (U8 reg, U8 value)
{
   while(RTC0ADR & 0x80);			   //查询 BUSY（RTC0ADR.7）直到其返回 0 值
